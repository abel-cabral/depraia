...
public class BanhistaController {
    ...
    @PostMapping(path = "")
    public @ResponseBody
    Map<String, Boolean> addEntity(@NotNull @Valid @RequestBody BanhistaDTO entity) {        
        try {
            // Busca agenda pelo ID
            return agendaRepo.findById(entity.getAgenda().getId())
                    .map(n -> {
                        // Preparar                        
                        Banhista aux = entity.conversor(n);
    ...
    @PutMapping("/{id}")
    public @ResponseBody
    Map<String, Boolean> updateById(@NotNull @Valid @RequestBody BanhistaDTO entity, @PathVariable int id) {
        try {
            // Busca agenda pelo ID                                
            return agendaRepo.findById(entity.getAgenda().getId()).map(n -> {
                ...
                return banhistaRepo.findById(id)
                        ...
                        .orElseGet(() -> {
                            return Mensagem.error("Banhista", 4);
                        });
            })
                    .orElseGet(() -> {
                        return Mensagem.error("Agenda", 4);
    ...
    @DeleteMapping("/{id}")
    public @ResponseBody
    Map<String, Boolean> deleteById(@NotNull @Valid @RequestBody BanhistaDTO entity, @PathVariable int id) {
        try {
            // Busca agenda pelo ID                               
            return agendaRepo.findById(entity.getAgenda().getId()).map(n -> {
                // Busca no banco de dados
